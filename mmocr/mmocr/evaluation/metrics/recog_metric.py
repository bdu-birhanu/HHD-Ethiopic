...
class WordMetric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 valid_symbol: str = 'ሀሁሂሃሄህሆለሉሊላሌልሎሏሐሑሒሓሔሕሖመሙሚማሜምሞሟሠሡሢሣሤሥሦረሩሪራሬርሮሯሰሱሲሳሴስሶሷሸሹሺሻሼሽሾሿቀቁቂቃቄቅቆቈቊቋቍበቡቢባቤብቦቧቨቩቪቫቬቭቮተቱቲታቴትቶቷቸቹቺቻቼችቾቿኀኁኂኃኄኅኆኈኊኋኍነኑኒናኔንኖኗኘኙኚኛኜኝኞኟአኡኢኣኤእኦኧከኩኪካኬክኮኰኲኳኴኵኸኹኺኻኼኽኾወዉዊዋዌውዎዏዐዑዒዓዔዕዖዘዙዚዛዜዝዞዟዠዡዢዣዤዥዦየዩዪያዬይዮደዱዲዳዴድዶዷዿጀጁጂጃጄጅጆጇገጉጊጋጌግጎጐጒጓጔጕጠጡጢጣጤጥጦጧጨጩጪጫጬጭጮጯጰጱጲጳጴጵጶጸጹጺጻጼጽጾጿፀፁፂፃፄፅፆፈፉፊፋፌፍፎፏፐፑፒፓፔፕፖፚ፠፡።፣፤፥፦፨፩፪፫፬፭፮፯፰፱፲፳፴፵፶፷፸፹፺፻፼',
                 collect_device: str = 'cpu',
                 prefix: Optional[str] = None) -> None:
        super().__init__(collect_device, prefix)
        self.valid_symbol = valid_symbol  #re.compile(valid_symbol)
...
class OneMinusNEDMetric(BaseMetric):
    ...
    def process(self, data_batch: Sequence[Dict],
        ...
        for data_sample in data_samples:
            ...
            #gt_text_lower = gt_text.lower()
            #pred_text_lower = pred_text.lower()
            gt_text_lower_ignore = self.valid_symbol.sub('', pred_text)
            pred_text_lower_ignore = self.valid_symbol.sub('', gt_text)
            #norm_ed = Levenshtein.normalized_distance(pred_text_lower_ignore,
                                                      #gt_text_lower_ignore)
            norm_ed = Levenshtein.distance(pred_text_lower_ignore,
                                                      gt_text_lower_ignore)
