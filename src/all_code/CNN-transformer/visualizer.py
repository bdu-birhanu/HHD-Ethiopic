...
def draw_predicted_text(label, pred_label, fontdict, width, height):
    ...
    if pred_label == label:
        ...
        plt.text(width, height, pred_label, fontdict=fontdict)
        return 

    pred_start, start, end = 0, 0, 0
    y = height / 2
    for i, c in enumerate(label):
        actual_char = '[UNK]' if c == '?' else c

        if label[start:end + 1] in pred_label[pred_start:pred_length]:
            fontdict['color'] = 'dodgerblue'
            x = (i + 0.5) * width / label_length
            plt.text(x, y, actual_char, fontdict=fontdict)
        else:
            if end < pred_length and end + 1 < label_length and pred_label[end] == label[end + 1]:
                ...
                fontdict['color'] = 'red'
                x = (i + 0.5) * width / label_length
                plt.text(x, y, pred_label[end], fontdict=fontdict)
                fontdict['color'] = 'black'
                plt.text(x, y + height / 4, actual_char, fontdict=fontdict)
            else: 
                fontdict['color'] = 'gray'
                x = (i + 0.5) * width / label_length
                plt.text(x, y, actual_char, fontdict=fontdict)
...
def visualize_images_labels(
    ...
    for i in range(min(nrows * ncols, num_of_labels)):
        ...
        if pred_labels: 
            draw_predicted_text(label, pred_labels[i], fontdict, width, height)
        else: 
            plt.text(0, height + 10, ' '.join(label), fontdict=fontdict)
        plt.axis('off')
