...
def func_model():
    ...
    early_stopping_callback = EarlyStopping(
        ...
        patience = 15, # Stop if no improvement after 7 epochs
    ...
    model.save_weights(path + f'model_cnn_transformer_{i}.h5')
    #model.save_model('./noname_full')
...
early_stopping_callback = EarlyStopping(
    monitor = 'val_loss', 
    min_delta = 1e-3, # Change that less than 1e-3, will count as no improvement
    patience = 10, # Stop if no improvement after 7 epochs
    restore_best_weights = True, # Restore weights from the epoch with the best value
    verbose = 1
)
...
for i in range(rounds):
    model_call=func_model()
    model_call.save_weights(path + f'model_cnn_transformer_{i}.h5')
    true=[]
    pred=[]
    batch_results = []
    ...
    for idx, (batch_images, batch_tokens) in enumerate(tf_dataset_rand):
        ...
        pred_tokens, attentions = model_call.predict(batch_images, return_attention=True)
    ...
    print(f'this is training transformer round_{i}')
print("CER rand:")
print (CER_all_rand)
print("CER 18th:")
print (CER_all_18th)
